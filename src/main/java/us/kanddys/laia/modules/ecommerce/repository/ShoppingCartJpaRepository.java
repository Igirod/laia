...
public interface ShoppingCartJpaRepository extends JpaRepository<ShoppingCart, Long> {
   ...
   @Query(value = "SELECT * FROM shopping_carts WHERE merchant_id = :merchantId AND invoice_id = :invoiceId", nativeQuery = true)
   public Optional<ShoppingCart> existByMerchantIdAndUserId(Long merchantId, Long invoiceId);
}
